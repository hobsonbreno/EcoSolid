...
export class DashboardController {
  ...
  @Get('metricas')
  async getMetricas(@Query('partnerName') partnerName: string, @Query('periodo') periodo?: string) {
    try {
      ...
      const [totalResgates, aprovados, rejeitados, solidAgg] = await Promise.all([
        this.redemptionModel.countDocuments(baseFilter),
        this.redemptionModel.countDocuments({ ...baseFilter, status: { $in: ['CONFIRMADO', 'validated'] } }),
        this.redemptionModel.countDocuments({ ...baseFilter, status: { $in: ['EXPIRADO', 'expired'] } }),
      ...
      const taxaAprovacao = totalResgates > 0 ? Math.round((aprovados / totalResgates) * 100) : 0;

      return {
        success: true,
        data: { totalResgates, aprovados, rejeitados, solidDistribuido, taxaAprovacao },
      };
    } catch (error: any) {
  ...
  async getResgates30Dias(@Query('partnerName') partnerName: string) {
    try {
      ...
      const dataMap: Record<string, { data: string; aprovados: number; rejeitados: number }> = {};
      for (let i = 29; i >= 0; i--) {
      ...
      for (const r of results) {
        ...
        if (dataMap[key]) {
          if (['CONFIRMADO', 'validated'].includes(r._id.status)) dataMap[key].aprovados += r.count;
          else if (['EXPIRADO', 'expired'].includes(r._id.status)) dataMap[key].rejeitados += r.count;
        }
      }
  ...
  @Get('top-beneficios')
  async getTopBeneficios(@Query('partnerName') partnerName: string, @Query('periodo') periodo?: string) {
    try {
      const since = this.getDateRange(periodo || 'hoje');
      const match: any = {};
      ...
      const results = await this.redemptionModel.aggregate([
        { $match: match },
        { $group: { _id: '$benefitDescription', quantidade: { $sum: 1 }, solidTotal: { $sum: '$solidCost' } } },
        { $sort: { quantidade: -1 } },
        { $limit: 10 },
      ]);
  ...
  async getUltimosResgates(@Query('partnerName') partnerName: string) {
    try {
      ...
      const data = await this.redemptionModel.find(filter).sort({ createdAt: -1 }).limit(10).lean().exec();
      return { success: true, data };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  ...
  async getStatusSistema() {
    try {
      const [totalParceiros, totalUsuarios, solidAgg, segmentosAgg] = await Promise.all([
        this.partnerModel.countDocuments({ ativo: true }),
        this.citizenModel.countDocuments(),
        this.citizenModel.aggregate([{ $group: { _id: null, total: { $sum: '$totalPoints' } } }]),
