...
export class CitizenExtratoController {
  ...
  async getExtrato(
    ...
    try {
      if (!usuarioId)
        return { success: false, error: 'usuarioId é obrigatório' };
      ...
      const buildUserFilter = (fieldOrigem: string, fieldDestino: string) => {
        const f: any = {
          ...baseFilter,
          $or: [{ [fieldOrigem]: usuarioId }, { [fieldDestino]: usuarioId }],
        };
        if (busca) {
          f.$and = [
            {
      ...
      const fetchPix =
        tipo !== 'crypto'
          ? this.pixModel
              .find(buildUserFilter('usuarioOrigemId', 'usuarioDestinoId'))
              .lean()
              .exec()
          : Promise.resolve([]);
      const cryptoFilter = buildUserFilter('from', 'to');
      if (walletAddress)
        cryptoFilter.$or = [{ from: walletAddress }, { to: walletAddress }];
      const fetchCrypto =
        tipo !== 'pix'
          ? this.cryptoModel.find(cryptoFilter).lean().exec()
          : Promise.resolve([]);
      ...
      const pixItems = pixData.map((p) => ({
        _id: p._id,
        tipo: 'pix',
        direcao: p.usuarioOrigemId === usuarioId ? 'saida' : 'entrada',
        valor: p.valor,
        moeda: p.moeda,
        contraparte:
          p.usuarioOrigemId === usuarioId
            ? p.nomeDestino || p.chaveDestino
            : p.nomeOrigem || p.chaveOrigem,
        descricao:
          p.usuarioOrigemId === usuarioId ? 'Enviado para' : 'Recebido de',
        status: p.status,
        hash: p.idTransacao,
        createdAt: p.createdAt,
        chaveOrigem: p.chaveOrigem,
        chaveDestino: p.chaveDestino,
      }));
      ...
      const cryptoItems = cryptoData.map((c) => ({
        _id: c._id,
        tipo: 'crypto',
        direcao: c.from === cryptoAddr ? 'saida' : 'entrada',
        valor: c.valor,
        moeda: c.moeda,
        contraparte:
          c.from === cryptoAddr
            ? c.to?.slice(0, 10) + '...' + c.to?.slice(-6)
            : c.from?.slice(0, 10) + '...' + c.from?.slice(-6),
      ...
      const combined = [...pixItems, ...cryptoItems].sort(
        (a, b) =>
    ...
    try {
      ...
      const [pixData, cryptoData] = await Promise.all([
        this.pixModel
          ...
          .exec(),
        this.cryptoModel
          .find({ ...baseFilter, $or: [{ from: wAddr }, { to: wAddr }] })
          .lean()
          .exec(),
      ]);
      ...
      return {
        ...
        data: {
          brl: {
            totalEntradas: totalEntradasBRL,
            totalSaidas: totalSaidasBRL,
            saldo: totalEntradasBRL - totalSaidasBRL,
          },
          eth: {
            totalEntradas: totalEntradasETH,
            totalSaidas: totalSaidasETH,
            saldo: totalEntradasETH - totalSaidasETH,
          },
        },
      };
    } catch (error: any) {
  ...
  async saveCrypto(
    ...
    try {
      ...
      return {
        success: true,
        data: tx,
        message: 'Transação crypto registrada',
      };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  ...
  async confirmarCrypto(@Param('hash') hash: string) {
    try {
  ...
  async verificarPendentes() {
    try {
      ...
      const pendentes = await this.cryptoModel
      ...
      for (const tx of pendentes) {
        ...
        if (!tx.hash || !tx.hash.startsWith('0x') || tx.hash.length !== 66) {
          ...
          if (age > 24 * 60 * 60 * 1000) {
            await this.cryptoModel.updateOne(
              { _id: tx._id },
              { status: 'falhou' },
            );
            atualizados++;
      ...
      return {
        success: true,
        message: `${atualizados} transações atualizadas de ${pendentes.length} pendentes`,
      };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
