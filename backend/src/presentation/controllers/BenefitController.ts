...
export class BenefitController {
  ...
  private readonly benefitMap: Record<
    string,
    { segmento: string; orgao: string; duracaoMinutos: number }
  > = {
    'Zona Azul Fortaleza': {
      segmento: 'Estacionamento',
      orgao: 'Prefeitura Municipal de Fortaleza - AMC',
      duracaoMinutos: 60,
    },
    'Clinica Saude+': {
      segmento: 'Hospital',
  ...
  async redeem(
    ...
    try {
      ...
      const meta = this.benefitMap[body.partnerName] || {
        segmento: 'Outro',
        orgao: body.partnerName,
        duracaoMinutos: 0,
      };
      console.log(
        '[BenefitRedeem] Parceiro:',
        body.partnerName,
        '→ Segmento:',
        meta.segmento,
        'Orgao:',
  ...
  async validate(@Body() body: { code: string; partnerName: string }) {
    try {
      ...
      if (citizen) {
        citizen.totalPoints = Math.max(
          0,
          (citizen.totalPoints || 0) - redemption.solidCost,
        );
      ...
      if (redemption.duracaoMinutos > 0) {
        redemption.expiresAt = new Date(
          now.getTime() + redemption.duracaoMinutos * 60000,
        );
  ...
  async confirmRedemption(
    ...
    try {
      ...
      if (redemption.status === 'CONFIRMADO')
        return { success: false, error: 'Já confirmado' };
  ...
  async debugAll() {
    try {
      const all = await this.redemptionModel
        .find()
        .sort({ createdAt: -1 })
        .limit(50)
        .lean()
