...
export class HistoricoController {
  ...
  private async getCnpjByPartnerName(partnerName: string): Promise<string> {
    ...
    const p = await this.partnerModel
      .findOne({ nomeFantasia: partnerName })
      .lean()
      .exec();
    return p?.cnpj || '';
  }
  ...
  async getExtrato(
    ...
    try {
      ...
      const buildSearch = (fields: string[]) =>
        busca
          ? {
              $or: fields.map((f) => ({
                [f]: { $regex: busca, $options: 'i' },
              })),
            }
          : {};
      ...
      if (busca) {
        ...
        cryptoFilter = {
          ...cryptoFilter,
          ...buildSearch(['hash', 'from', 'to']),
        };
      }

      const fetchPix =
        tipo !== 'crypto'
          ? this.pixModel.find(pixFilter).lean().exec()
          : Promise.resolve([]);
      const fetchCrypto =
        tipo !== 'pix'
      ...
      const pixItems = pixData.map((p) => ({
        ...
        contraparte:
          p.direcao === 'entrada'
            ? p.nomePagador || p.chave
            : p.nomeRecebedor || p.chave,
        descricao:
          p.direcao === 'entrada'
      ...
      const totalEntradas = combined
        .filter((i) => i.direcao === 'entrada')
        .reduce((s, i) => s + i.valor, 0);
      const totalSaidas = combined
  ...
  async savePix(
    ...
    try {
      const tx = await this.pixModel.create({
        ...body,
        tipo: 'pix',
        moeda: 'BRL',
      });
