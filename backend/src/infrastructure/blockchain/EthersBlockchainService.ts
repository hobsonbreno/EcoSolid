...
export class EthersBlockchainService implements IBlockchainService {
  ...
  constructor() {
    ...
    try {
      ...
      this.ready = true;
      console.log(
        `[Blockchain] Conectado à Sepolia. Wallet: ${this.wallet.address}`,
      );
    } catch (err) {
      console.warn(
        '[Blockchain] Aviso: Falha na inicialização. Transações serão simuladas.',
        err,
      );
    }
  }
  ...
  async registerAction(
    ...
    if (!this.ready) {
      ...
      console.log(
        `[Blockchain] MOCK — hash simulado (sem configuração): ${mockHash}`,
      );
      return mockHash;
    }
    ...
    try {
      ...
      console.log(
        `[Blockchain] registerAction — actionId: ${actionIdBytes}, citizen: ${addr}, points: ${points}, type: ${actionType}`,
      );
      ...
      console.error(
        '[Blockchain] Falha ao registrar ação na blockchain:',
        error.message,
      );
      ...
      const pendingHash =
        ...
        Math.random().toString(36).substring(2, 10);
      console.log(
        `[Blockchain] Hash pendente (retry posterior): ${pendingHash}`,
      );
      return pendingHash;
    }
  }
  ...
  async confirmRedemption(
    ...
    if (!this.ready) {
      const mockHash =
        ...
        Math.random().toString(36).substring(2, 10);
      console.log(
        `[Blockchain] MOCK redemption — hash simulado (sem configuração): ${mockHash}`,
      );
      return mockHash;
    }
    ...
    try {
      ...
      const addr = citizenAddress || this.wallet.address;
      console.log(
        `[Blockchain] confirmRedemption — id: ${redemptionIdBytes}, citizen: ${addr}, benefit: ${benefit}, solid: ${solidSpent}`,
      );

      const tx = await this.contract.confirmRedemption(
        redemptionIdBytes,
        addr,
        benefit,
        solidSpent,
      ...
      console.error(
        '[Blockchain] Falha ao confirmar resgate na blockchain:',
        error.message,
      );
      const pendingHash =
        'pending-redemption-' +
