...
export class MongoCitizenRepository implements ICitizenRepository {
  ...
  private mapToDomain(doc: CitizenDocument): Citizen {
    return new Citizen(doc._id, doc.name, doc.walletAddress, doc.cpf, doc.email, doc.phone, doc.birthDate, doc.address, doc.facePhotoUrl, doc.totalPoints, doc.level, doc.createdAt, doc.credentialId, doc.credentialPublicKey);
  }
  async save(citizen: Citizen): Promise<void> {
    await this.citizenModel.findOneAndUpdate({ _id: citizen.id }, { name: citizen.name, walletAddress: citizen.walletAddress, cpf: citizen.cpf, email: citizen.email, phone: citizen.phone, birthDate: citizen.birthDate, address: citizen.address, facePhotoUrl: citizen.facePhotoUrl, totalPoints: citizen.totalPoints, level: citizen.level, createdAt: citizen.createdAt, credentialId: citizen.credentialId, credentialPublicKey: citizen.credentialPublicKey }, { upsert: true, new: true }).exec();
  }
